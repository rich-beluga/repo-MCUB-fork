...
class CreateButtonsMod(ModuleBase):
    ...
    @command('create-button', doc={'ru': '<count> создать X количество кнопок', "en": "<count> create X count button(s)"})
    async def create_buttons(self, message: Any) -> None:
        try:
            args = int(utils.get_args_raw(message))
        except ValueError as err:
            self.log.debug(f'error debug: {err}')
            await utils.answer(message, f'please typing <code>{self.kernel.custom_prefix}create_buttons 1</code>', as_html=True)
            return
        buttons = []
        for i in range(int(args)):
            buttons.append([self.callback_button(f'button {i+1}', self.on_click)])
        await self.kernel.inline_form(
            message.chat_id,
            f'Buttons created: {args}',
            buttons=buttons
        )
    @callback(ttl=50)
