...
def register(kernel):
    class FHeta:
        ...
        async def client_ready(self):
            ...
            config_dict = await kernel.get_module_config(__name__, default_config)
            config = ModuleConfig(
                ConfigValue(
                    "tracking",
                    True,
                ...
                ConfigValue(
                    ...
                    validator=Choice(choices=["default"], default="default"),
                ),
            )
        ...
        def _fmt_mod(self, mod, query="", idx=1, total=1, inline=False):
            ...
            if total > 1:
                info = (
                    f"{self._get_emoji('result')} <b>{self._('result', idx=idx, total=total)}</b> <code>{self._escape_html(query)}</code>\n"
                    + info
                )
            elif query and not inline:
                info = (
                    f"{self._get_emoji('result')} <b>{self._('result_by_query')}</b> <code>{self._escape_html(query)}</code>\n"
                    + info
                )
            ...
            if desc:
                if isinstance(desc, dict):
                    text = (
                        desc.get(kernel.config.get("language", "ru"))
                        or desc.get("doc")
            ...
            if cmds:
                ...
                for cmd in cmds:
                    if not cmd.get("inline"):
                        ...
                        desc_text = (
                            desc_dict.get(kernel.config.get("language", "ru"))
                            or desc_dict.get("doc")
                            or ""
                        )
        ...
        def _mk_btns(self, install, stats, idx, mods=None, query=""):
            ...
            like_row.append(
                Button.inline(
                    f"{like_emoji} {stats.get('likes', 0)}", data=like_data.encode()
                )
            )
            like_row.append(
                Button.inline(
                    f"{dislike_emoji} {stats.get('dislikes', 0)}",
                    data=dislike_data.encode(),
                )
            )
            ...
            if mods and len(mods) > 1:
                ...
                if idx > 0:
                    ...
                    nav_buttons.append(
                        Button.inline(prev_emoji, data=nav_data.encode())
                    )
                if idx < len(mods) - 1:
                    nav_data = f"fheta_nav:{idx + 1}:{query}"[:64]
                    nav_buttons.append(
                        Button.inline(next_emoji, data=nav_data.encode())
                    )
                if nav_buttons:
    ...
    async def fheta_handler(event):
        ...
        if len(args) < 2:
            await event.edit(
                f"{fheta._get_emoji('error')} <b>{fheta._('enter_query')}</b>",
                parse_mode="html",
            )
            return
        ...
        if len(query) > 168:
            await event.edit(
                f"{fheta._get_emoji('warn')} <b>{fheta._('query_too_long')}</b>",
                parse_mode="html",
            )
            return
        ...
        if not kernel.is_bot_available():
            await event.edit(
                f"{fheta._get_emoji('error')} <b>{fheta._('inline_bot_not_available')}</b>",
                parse_mode="html",
            )
            return
        ...
        try:
            ...
            if not bot_username:
                await event.edit(
                    f"{fheta._get_emoji('error')} <b>{fheta._('inline_bot_not_configured')}</b>",
                    parse_mode="html",
                )
                return
            await event.edit(f"<b>{fheta._('searching_module')}</b>", parse_mode="html")
            ...
            if success:
                ...
                await event.edit(
                    f"{fheta._get_emoji('error')} <b>{fheta._('search_failed')}</b>",
                    parse_mode="html",
                )
            ...
            await event.edit(
                f"{fheta._get_emoji('error')} <b>{fheta._('command_error')}</b>",
                parse_mode="html",
            )
